...
export interface GroupSiblings {
  ...
  // Alignment guides: the offset takes the offset-changing PROJECTION of the
  // group delta (guideNudgeDelta: dy for a horizontal guide, dx for a
  // vertical, dy∓dx for the diagonals). An INFINITE guide has nothing else to
  // move — the along-axis half of the drag slides past it — but a bounded
  // one's span (startCenter, null when unbounded) takes exactly that half, so
  // the segment travels rigidly with the group.
  guides: {
    id: string;
  ...
  for (const id of shows.guides ? sel.selectedGuideIds : []) {
    ...
    if (g && !g.locked) {
      out.guides.push({
        ...
        startCenter: g.extent?.center ?? null,
      });
    }
  }
  return out;
...
export function translateSiblings(s: GroupSiblings, dx: number, dy: number): void {
  ...
  // Only the offset-changing projection of the delta moves a guide's LINE; a
  // bounded span takes the along-axis projection too, so the segment travels
  // rigidly (an infinite guide has no along position to move).
  for (const gs of s.guides) {
    doc.moveGuide(
      gs.id,
      gs.startOffset + guideNudgeDelta(gs.orientation, dx, dy),
      gs.startCenter === null
        ? undefined
        : gs.startCenter + guideAlongOf(gs.orientation, { x: dx, y: dy }),
    );
  }
}
