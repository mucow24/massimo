...
/**
 * Open the Canvas → Save flyout. The leaf must then be activated with
 * fireEvent.click, NOT user.click: the flyout is hover-driven and userEvent's
 * pointer movement tears it down before the click lands. The symptom is
 * "called 0 times", which is indistinguishable from "not built yet".
 */
const openSaveSubmenu = async (user: ReturnType<typeof userEvent.setup>) => {
...
describe('Toolbar — Canvas menu', () => {
  it('Save → JSON serializes and triggers a download', async () => {
    const user = userEvent.setup();
    render(<Toolbar />);
    await openSaveSubmenu(user);
...
describe('Toolbar — map library', () => {
  ...
  const saveToLibrary = async (user: ReturnType<typeof userEvent.setup>) => {
    await openSaveSubmenu(user);
    fireEvent.click(screen.getByRole('menuitem', { name: 'To library' }));
  };

  it('Save → To library writes a user revision of the serialized doc', async () => {
    const user = userEvent.setup();
    seedRealMap();
    vi.mocked(getCanvasSvg).mockReturnValue({} as SVGSVGElement);
    render(<Toolbar />);
