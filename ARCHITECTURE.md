...
`resolveTextStyle` reads weight/style/size/fill/letter-spacing off the attributes; `pickFace`
chooses the face (exact, else nearest weight in style, else any) and a glyph no main face covers
falls back to a symbol font (DejaVu). `glyphPathData` serializes with an explicit space between
