...
anchors; hosted ones are station internals and never appear here; line circles are only ever
single-selected from the canvas — no marquee membership — but keep the list shape). The six
generic lists' `select/toggle/set/add/xor`
...
explicit arm or a deliberate degrade-to-null like line circles, which have no popover and are
not in the deep-pick stack; the tail is a bare `return null`, since the old unguarded svgImage
