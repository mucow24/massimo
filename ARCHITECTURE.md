...
`HighlightedLineLayer`. A hovered STATION also gets the map-consistent `hover-zone` silhouette —
...
10. `HighlightedLineLayer` (line-edit dim wash + the selected line repainted above it; in Edit
    Stops also the hovered station's `hover-zone` silhouette and, for a hovered FOREIGN line, that
    whole line repainted at `HOVER_LINE_OPACITY`) → `LineTagsLayer`.
