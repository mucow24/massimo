...
`guideAdmitsFoot`, and the drawable
...
values keep only their own 45° family and grid-quantize the intercept under the full lattice).
A BOUNDED one is an object on the canvas rather than a line across it, so its drag is **2-DOF**:
that same offset move, plus a slide down its own street by the along projection
(`guideAlongOf` of the pointer delta, `extent.center` and the offset landing in one `moveGuide`
write). The along half snaps on the span's two TIPS — each through the point snapper constrained
ALONG the axis, the offset's constraint mirrored, so a slid tip and a swept one behave alike —
and the better-aligned of the two carries the whole span onto its target. A tip with nothing in
range ABSTAINS rather than voting for a zero displacement it would otherwise win every contest
with, which is what lets a tip sitting exactly on a target hold the span against a further one
pulling at it. The line wears the plain move cursor once bounded, in place of the perpendicular
one an infinite guide advertises. **Ctrl/Cmd is the
...
Releasing Ctrl resumes the offset drag re-based so nothing jumps — the span re-bases the same
way, and a guide the sweep just bounded gets its start along-position there — towed siblings
freeze during
...
- [customPalettes.ts](src/state/customPalettes.ts) — `useCustomPalettes`: the user's half of the
  palette **library** in **global** localStorage (`'massimo-custom-palettes-v1'`, version 2) —
  imported definitions, the starred NAMES (built-ins included, so a star outlives a list they
  aren't in), and the library column's sort. `addPalette` upserts by exact name and refuses two
  things: a built-in's name (the library is keyed by name, so nothing may appear twice) and a
  palette carrying no colors, which is not a palette. `v1 → v2` drops the ones already stored
  that way — see `dropEmptyPalettes` and the palette manager for whose habit put them there.
...
passes it (the Ctrl sweep, an end handle, a bounded slide's two tips), where a crossing guide
pins an end exactly where an end wants pinning and a parallel one is structurally out anyway, the
along `constrain` dropping the axis it would engage on. Offset-side, that pool is taken only to
MEASURE against
...
- **`capCenterDy(fontSize)`** places every **badge glyph** — a service code in a stop dot or route
  ...
  CoreText on macOS. Whenever a face leaves `USE_TYPO_METRICS` clear and its two sets disagree — a
  904/−214 against 714/−198 spread is typical — `central` lands 0.345em above the baseline but
...
`guideAxis` — so master and tow stay rigid whichever way the master moved. A master whose span
flips to infinite mid-gesture FREEZES that along carry rather than dropping it, since the tow it
already did is not undone by the guide losing its ends. Guides never join the rotate.
...
panel's own measured width (each shell sets its own, 280 to 372) 8px
...
- **[PalettesDialog.tsx](src/components/PalettesDialog.tsx)** — the palette manager, opened by the
  ...
  write at the drop. Under the map's palettes stands the one row the map does not carry: **custom
  colors** — the line colors no palette covers (`useCustomLineColors`, the same set the line
  picker's Custom section shows), derived from the doc rather than stored in it, and italic to say
  so. It is the one row that breaks the grid: a save arrow and a drag handle would both be lies, so
  those two slots stand EMPTY at their usual width (strips and `…` stay in their columns), and the
  row spends a third action slot on a **`+`** that mints those colors as a palette into library and
  map and opens the editor on it — which covers them, and so empties the row away. Its `…` holds
  export alone (under a plain name — the row's parenthetical label is not something a palette
  loaded back from the file could wear). The manager and the map library share one
  ...
  **[PaletteEditor](src/components/PaletteEditor.tsx)** view, a back arrow joining the title band:
  ...
  while there is nothing to choose between, and opens a MENU once there is: New, or one of the
  map's custom colors as a swatch in a flyout — the way a color picked by hand on the canvas is
  filed into a palette without being read off one window and typed into another. The offer is the
  ...
  transform — that would become the containing block for the ColorField popover's position:fixed).
