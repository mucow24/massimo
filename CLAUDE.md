...
- Gate with `npm run pre-pr`. On Windows (= Mike's machine, where several sessions may gate at
  once) it routes itself through a machine-wide kernel mutex
  ([scripts/preprQueued.ps1](scripts/preprQueued.ps1)) so parallel gates serialize instead of
  thrashing each other; on Linux (cloud containers, CI) it runs the chain directly.
  `pre-pr:queued` is a compatibility alias for the same thing. The perf harnesses in `.perf/`
  take the same mutex on their own and refuse to report numbers from a contended machine.
