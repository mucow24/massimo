...
/**
 * Click-to-edit rename state, shared by the inline name fields (map name, style
 * name in the Styles panel, "Save style…" in the style-preset row). Manages the
 * draft plus a one-shot cancel flag so a commit fires exactly once — on Enter or
 * blur — and Escape reverts without touching the store.
 *
