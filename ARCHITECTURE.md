...
rim-only. Being two grabs for ONE object, they share a hover: the mouseover preview is wired on
the wrapper `<g>`, so crossing from the rim to the ⊕ never reads as leaving the circle. Every part except that knob is a translation and tows the group. See the
...
- **Mouseover-preview twins.** Almost every selection-chrome layer has a hover twin mounted
  ...
  INSIDE `LineCircleView` instead: its selection chrome is a recolour of the guide, so the preview
  is a second copy of the guide marks in the accent, and it has to stay down in the guide layer
  under the map content rather than surface as a twin overlay.
